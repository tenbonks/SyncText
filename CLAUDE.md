...
- **Content-Security-Policy** is enforced via a `<meta http-equiv>` tag in `index.html` (static hosts like GitHub Pages can't send HTTP headers). It is `default-src 'none'` plus a narrow allowlist: `script-src` self + `unpkg.com`, `style-src`/`font-src` for Google Fonts, and `connect-src` for `0.peerjs.com` (the signaling broker). WebRTC's own STUN/data traffic is not gated by `connect-src` in current browsers, so it isn't listed. Adding any new origin, inline script, or inline style requires updating this policy or it will be silently blocked.
