...
- **PeerJS over WebRTC** ([index.html](index.html) loads it from the `unpkg` CDN) brokers the P2P connection via PeerJS's default public signaling server; there is no backend of our own. This CDN script is the only external dependency. The `<script>` tag carries a `sha384` Subresource Integrity hash — **if you bump the PeerJS version you must recompute it** (`curl -sSL <url> | openssl dgst -sha384 -binary | openssl base64 -A`, prefix `sha384-`) or the browser will refuse to load the script.
- **Content-Security-Policy** is enforced via a `<meta http-equiv>` tag in `index.html` (static hosts like GitHub Pages can't send HTTP headers). It is deliberately **relaxed for WebRTC**: `default-src 'self'`, with `script-src` self + `unpkg.com`, `style-src`/`font-src` for Google Fonts, `img-src 'self'`, every unused fetch type pinned to `'none'` (`object-src`/`frame-src`/`worker-src`/`manifest-src`/`media-src`), `base-uri 'none'`, `form-action 'none'`, and a broad `connect-src 'self' https: wss: stun: turn: turns:`. The unused types are set to `'none'` individually rather than via a blanket `default-src 'none'` — the blanket form caught something WebRTC-adjacent on Firefox and broke the connection, so it's avoided. The real anti-injection protection here is `script-src` (self + the SRI-pinned CDN) — that's what stops foreign/injected script executing. `connect-src` is intentionally broad because **Firefox — unlike Chromium — applies CSP to WebRTC's outward connections (ICE/STUN/TURN)**, so an earlier `default-src 'none'` with a narrow `connect-src` worked in Chrome but broke the P2P connection on Firefox with "ICE failed / TURN server appears to be broken" and a `default-src` violation. A P2P app must open outward connections by design, so a tight `connect-src` fights the app's purpose; keeping `script-src` locked preserves the protection that matters. **Test CSP changes in Firefox, not just Chromium** — Chromium won't surface this class of WebRTC/CSP bug. Adding any new script/style/font origin still requires updating this policy or it will be silently blocked.
