...
- **Content-Security-Policy** is enforced via a `<meta http-equiv>` tag in `index.html` (static hosts like GitHub Pages can't send HTTP headers). It is `default-src 'none'` plus a narrow allowlist: `script-src` self + `unpkg.com`, `style-src`/`font-src` for Google Fonts, and `connect-src` for the signaling broker (`0.peerjs.com`) plus the `stun:`/`turn:`/`turns:` schemes. **Firefox gates WebRTC ICE (STUN/TURN) through `connect-src`; Chromium does not** — so without the `stun:`/`turn:` schemes the P2P connection works in Chrome but fails on Firefox with "ICE failed / TURN server appears to be broken" and a `default-src` violation. PeerJS's default ICE servers are `stun:stun.l.google.com:19302` and `turn:{eu,us}-0.turn.peerjs.com:3478`; the scheme allowlist covers them without pinning hosts that PeerJS may rotate. Test CSP changes in Firefox, not just Chromium, or you'll miss this class of bug. Adding any new origin, inline script, or inline style requires updating this policy or it will be silently blocked.
