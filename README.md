...
- **You rely on two third parties:** PeerJS's public signaling broker (which sees connection metadata — peer ids — but not your DTLS-encrypted content) and the `unpkg` CDN that serves the PeerJS library. A compromised CDN could serve malicious code; the script is not pinned with Subresource Integrity. For a hardened setup, self-host the PeerJS library and run your own [PeerServer](https://github.com/peers/peerjs-server).
