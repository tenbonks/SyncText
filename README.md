...
- **The page ships a strict Content-Security-Policy.** A `<meta http-equiv="Content-Security-Policy">` tag restricts the page to exactly the origins it needs — its own files, the pinned PeerJS CDN, Google Fonts, and the PeerJS broker — and blocks everything else (`default-src 'none'`). This limits the blast radius if any injection ever occurred. It's delivered via `<meta>` rather than an HTTP header because the app is designed to run on a static host (e.g. GitHub Pages) that can't set response headers.
