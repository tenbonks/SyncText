...
4. Once the status dot turns green, type or paste in either pad, and it syncs live
...
It's a static page with no build step and no server of your own. Open `index.html` directly in a browser, or serve the folder statically (recommended, since some browsers block the CDN script over `file://`):
...
- **The page ships a Content-Security-Policy.** A `<meta http-equiv="Content-Security-Policy">` tag locks down what the page can load and run: scripts only from its own origin and the pinned PeerJS CDN, styles/fonts only from Google Fonts, no plugins/objects, no `<base>` hijack. The script restriction is the important one; it's what stops injected or foreign code executing. Outbound connections (`connect-src`) are left broad on purpose: a peer-to-peer app has to open connections to peers, STUN and TURN servers, and **Firefox, unlike Chromium, applies CSP to those WebRTC connections**, so a tight policy silently broke the P2P link on Firefox ("ICE failed / TURN server appears to be broken"). Loosening `connect-src` costs little here because the script lock stays in place and your pad text is peer-to-peer by design anyway. It's delivered via `<meta>` rather than an HTTP header because the app runs on a static host (e.g. GitHub Pages) that can't set response headers.
